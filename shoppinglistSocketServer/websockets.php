...
abstract class WebSocketServer {
  ...
  protected $db;  // Axel
  protected $commandFactory; // Axel
  protected $fictionalDelay; // Axel

  function __construct($addr, $port, $db = null, $commandFactory = null, $fictionalDelay, $bufferLength = 2048) {
    // AXEL:
    if ($db != null && $db instanceof Database) {
      $this->db = $db;
      if ($commandFactory != null && $commandFactory instanceof CommandFactory) {
        $this->commandFactory = $commandFactory;
      } else {
        echo "VARNING: Inget CommandFactory-objekt mottaget.";
      }
    } else {
      echo "VARNING: Inget Database-objekt mottaget.";
    }
    if (isset($fictionalDelay)) {
        $this->fictionalDelay = $fictionalDelay;
    }
    ...
    while(true) {
      ...
      foreach ($read as $socket) {
        if ($socket == $this->master) {
          ...
          if ($numBytes === false) {
            /*  Patch för att förhindra krasch @author mail@axellarsson.nu
             *  Om en klient har droppat och sedan återanslutet sig själv med
             *  en ny anslutning utan att vi vet att det är samma klient så
             *  kunde innan en krasch uppstå när den "nya" anslutningen skickade
             *  ett meddelande. Nu avslutar vi helt enkelt den gamla anslutningen
             *  istället och fortsätter som vanligt.
             */
            $this->disconnect($socket);
            $this->stdout("Client disconnected. TCP connection lost: " . $socket);
            // Gammal kod var: throw new Exception('Socket error: ' . socket_strerror(socket_last_error($socket)));
          }
          elseif ($numBytes == 0) {
            $this->disconnect($socket);
            $this->stdout("Client disconnected. TCP connection lost: " . $socket);
